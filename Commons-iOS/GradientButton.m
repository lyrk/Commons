...
- (void)drawRect:(CGRect)rect 
{
	...
	//draw percent bar
	//_progressNormal = 0.5; //for testing
	if ((_progressNormal > 0)){
		//CGContextSetBlendMode(context,  kCGBlendModeOverlay);
		CGContextSetBlendMode(context,  kCGBlendModeHardLight);
		
		UIColor *c = [UIColor colorWithRed:0.0 green:0.0 blue:1.0 alpha:0.5];
		CGColorRef redColor = c.CGColor;
		CGContextSetFillColorWithColor(context, redColor);
		
		CGRect b = CGRectInset(self.bounds, 1, 0);
		b = CGRectMake(b.origin.x, b.origin.y, b.size.width * _progressNormal, b.size.height);
